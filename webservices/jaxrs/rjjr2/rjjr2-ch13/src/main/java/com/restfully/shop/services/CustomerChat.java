...
public class CustomerChat {
	...
	protected LinkedHashMap<String, Message> messages = new LinkedHashMap<String, Message>() {
		private static final long serialVersionUID = -6899678858835942800L;
	...
	public void post(final String text) {
		...
		System.out.println("Path parameters: " + uriInfo.getPathParameters());
		System.out.println("Path segments: " + uriInfo.getPathSegments());
		System.out.println("Request URI: " + uriInfo.getRequestUri());
		
		writer.submit(new Runnable() {
			@Override
			public void run() {
				System.out.println("Entering POST synchronized block...");
				...
				System.out.println("Exited POST synchronized block...");
			}
		});
	}
	...
	public void receive(@QueryParam("current") String next,
		...
		System.out.println("Entering GET synchronized block...");
		synchronized (messages) {
		...
		System.out.println("Exited GET synchronized block");
